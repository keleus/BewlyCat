...
/** Keep browser-created line breaks while treating mentions/emotes as atomic nodes. */
...
/** Match only the unfinished trigger before the caret, never inside an existing token. */
export function findRepostCompletion(root: HTMLElement, caret: Range) {
  if (!caret.collapsed || !root.contains(caret.startContainer))
    return null
