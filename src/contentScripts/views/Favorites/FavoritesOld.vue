  ...
  try {
    ...
    if (res.code === 0) {
      ...
      if (Array.isArray(res.data.medias) && res.data.medias.length > 0)
        favoriteResources.push(...res.data.medias.filter((item): item is FavoriteItem => item != null))
  ...
  <div v-if="getCSRF()" flex="~ col md:row lg:row" gap-6>
    <main w="full md:60% lg:70% xl:75%" order="2 md:1 lg:1" relative>
      <div
        ...
        items-center
        bg="$bew-elevated-solid" rounded="$bew-radius" shadow="$bew-shadow-2" mt--2 transition="background-color duration-300, box-shadow duration-300, transform duration-300"
