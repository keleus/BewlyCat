...
export function rememberBewlyWidescreenMode(enabled: boolean) {
  localSettings.value.bewlyWidescreenModePreferred = enabled
}
...
function applyCustomSidebarWidth(width: number, persist = false) {
  ...
  const normalizedWidth = settings.value.enableBewlyWidescreenSidebarResize
  ...
  if (!normalizedWidth || window.innerWidth <= MOBILE_BREAKPOINT) {
    ...
    const clampedWidth = Math.round(Math.min(Math.max(normalizedWidth, minWidth), maxWidth))
    state.root.dataset.sidebarCustomWidth = 'true'
    state.root.style.setProperty('--bewly-widescreen-sidebar-custom-width', `${clampedWidth}px`)
    ...
    state.sidebarResizeHandle.setAttribute('aria-valuenow', String(clampedWidth))
  }
...
function updateSidebarLayoutState() {
  ...
  const visibleSidebarWidth = hasCustomWidth
    ...
    : state.sidebarMode === 'narrow'
      ? narrowWidth
      : wideVideoPriority
        ? SIDEBAR_VIDEO_PRIORITY_COLLAPSED_WIDTH
        : fitWidth
  ...
  const needsHover = !hasCustomWidth
    && state.sidebarMode === 'fit'
    && (wideVideoPriority || narrowWidth - fitWidth > 1)
...
function applyCustomSidebarWidthStyle(currentState: BewlyWidescreenState) {
  ...
  currentState.sidebarResizeHandle.setAttribute('aria-valuemin', String(Math.round(minWidth)))
  currentState.sidebarResizeHandle.setAttribute('aria-valuemax', String(Math.round(maxWidth)))
  currentState.sidebarResizeHandle.setAttribute('aria-valuenow', String(clampedWidth))
}
...
function setupSidebarResize(currentState: BewlyWidescreenState) {
  ...
  function onPointerDown(event: PointerEvent) {
    if (event.button !== 0 || !event.isPrimary || window.innerWidth <= MOBILE_BREAKPOINT)
      return
...
export function exitBewlyWidescreen(
  ...
  if (options.userInitiated)
    rememberBewlyWidescreenMode(false)
